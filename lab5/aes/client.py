...
def key_to_keym(key:str) -> list[str]:
    keym = [key[i:i+4] for i in range(0, len(key), 4)]
    return keym

def sub_bytes(key: list[str]) -> list[str]:
    

def shift_rows(keym: list[str]) -> list[str]:
    for row in range(4):
...
key = "AB31AC9381333B31"
keym = key_to_keym(key)
keym_shifted = shift_rows(keym)
...
# tests
assert key_to_keym("AB31AC9381333B31") == ['AB31', 'AC93', '8133', '3B31'] , "key to keym not working"

assert shift_rows(['AB31', 'AC93', '8133', '3B31']) == ['AB31', 'C93A', '3381', '13B3'], "shift rows not working"
